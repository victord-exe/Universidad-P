...
CREATE OR REPLACE PROCEDURE InsertarSucursal (
    ...
    INSERT INTO Sucursal (cod_sucursal, nombre_sucursal, montoprestamo)
        VALUES (p_cod_sucursal, p_nombre_sucursal, p_montoprestamo);
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            -- de clave primaria duplicada
            DBMS_OUTPUT.PUT_LINE('La clave primaria ya existe. No se puede insertar.');
...
BEGIN
  FOR r_pago IN c_pagos LOOP
    ...
    -- Actualizar saldo actual del préstamo
    ...
    -- Actualizar interes_pagado del préstamo
    UPDATE Prestamo
    SET interes_pagado = interes_pagado + v_interes
    WHERE numero_prestamo = r_pago.prestamo AND cliente = r_pago.cliente;

    -- Actualizar montoprestamo de la sucursal
    UPDATE Sucursal
    SET montoprestamo = montoprestamo + r_pago.monto_pago
    WHERE cod_sucursal = r_pago.sucursal;

    DBMS_OUTPUT.PUT_LINE('transaccion_id: ' || r_pago.transaccion_id || ',  cliente: ' || r_pago.cliente || ',  prestamo: ' || r_pago.prestamo || ',  monto_pago: ' || r_pago.monto_pago || ',  sucursal: ' || r_pago.sucursal || ',  interes:' || v_interes || ',  monto_pendiente: ' || v_monto_pendiente || ',  saldo: ' || v_saldo);
  END LOOP;
...
DECLARE
    plata_suc Sucursal.montoprestamo%TYPE;
BEGIN
    plata_suc := 1000;
    UPDATE Sucursal
        SET montoprestamo = plata_suc
